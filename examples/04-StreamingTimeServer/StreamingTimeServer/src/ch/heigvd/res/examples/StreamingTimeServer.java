...
/**
 * A very simple example of TCP server. When the server starts, it binds a server
 * socket on any of the available network interfaces and on port 2205. It then waits
 * until one (only one!) client makes a connection request. When the client arrives,
 * the server does not even check if the client sends data. It simply writes the
 * current time, every second, during 15 seconds.
 * 
 * To test the server, simply open a terminal, do a "telnet localhost 2205" and see
 * what you get back. Use Wireshark to have a look at the transmitted TCP segments.
 * 
...
public class StreamingTimeServer {
	...
	public void start() {
		...
		try {
			LOG.log(Level.INFO, "Creating s server socket and binding it on any of the available network interfaces and on port {0}", new Object[]{Integer.toString(listenPort)});
